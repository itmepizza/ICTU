...
function RegisterHousingModal({ isDarkMode, session, rooms, buildings, occupiedMap, onClose, onRegistered }) {
  ...
  const handleSubmitChangeRequest = async () => {
    setChangeError('');
    const validationError = validateChangeRequest(currentResidency, changeReason);
    if (validationError) {
    ...
    const { data: newIssue, error: err } = await supabase.from('issues')
      .insert(buildRoomChangeIssuePayload(session.user.id, currentResidency, changeType, changeReason))
      .select().single();
  ...
  const vacantRooms = filterVacantRooms(rooms, occupiedMap);

  const manualResults = filterRoomsByCriteria(vacantRooms, { building: fBuilding, gender: fGender, type: fType });
...
function FeesView({ isDarkMode }) {
  ...
  const withDebt = rows.map((r) => ({ ...r, debt: calcDebt(r.monthlyFee, r.paid) }));
  ...
  const filtered = applyFeeFilters(withDebt, { statusFilter, search });
